...
> 구현: [`scripts/harness/workflows/orchestrate-feature.workflow.js`](../../../scripts/harness/workflows/orchestrate-feature.workflow.js). 근거: [RESEARCH_HARNESS_AND_MULTIAGENT.md](../research/RESEARCH_HARNESS_AND_MULTIAGENT.md).
...
orchestrate-feature가 *정해진 한 기능*을 정적 분해한다면, 감독자는 **대상을 런타임에 *발견*하고 동적 분배**한다 — 항목 수를 미리 모르는 배치/마이그레이션용. 구현: [`scripts/harness/workflows/supervise-batch.workflow.js`](../../../scripts/harness/workflows/supervise-batch.workflow.js).
...
- 검증 단계(④)는 단일 적대자. 더 엄격히 하려면 채널 B 다수결 패널([verify-findings-grounded](../../../scripts/harness/workflows/verify-findings-grounded.workflow.js))로 교체 가능.
