...
| `review.footguns` | **적대자가 항상 점검할 *스택-특정* 실패모드**(컴파일은 되나 런타임에 깨지는 것). ⚠️ **반드시 교체** | `@Transactional` 자기호출·JPA N+1·Kafka ack… (Spring) |

> `map`은 **첫 매치 우선**(위→아래). 새 프로젝트의 레이어/패키지 컨벤션에 맞춰 글롭과 문서명을 다시 쓴다.
> `review.footguns`는 **스택-특정**이라 그대로 두면 안 된다 — 새 스택의 함정으로 교체(예: Python/Django면 ORM lazy 쿼리·async-in-sync·N+1). 이 목록을 *프로토콜 산문이 아니라 config에 둔 이유*가 바로 이식성(프로토콜은 "config의 footgun을 점검하라"는 메커니즘만 보유 → 스택 무관).
