...
- **스택 footguns (필수 — `harness.config.json`의 `review.footguns`)**: *컴파일은 되지만 런타임에 깨지는* 실패모드를 각각 확인하라. 예(AnchorIQ=Spring): `@Transactional` private/자기호출 → 프록시 우회로 트랜잭션 무효, `@Async`/`@Cacheable` 자기호출, JPA N+1/lazy 초기화, Kafka 수동 ack 누락. **이 목록을 이 프로토콜에 *산문으로 박지 않은* 이유 = 스택-특정이라 박으면 하네스가 Java 전용이 된다.** footgun *내용*은 `review.footguns`(데이터, 프로젝트마다 교체)에 두고, 여기선 "config의 footgun을 각각 점검하라"는 *메커니즘*만 둔다. (근거: eval/results/DRIFT.md — 적대자가 self-invocation 버그를 잡았으나 '모델이 마침 앎'에 의존했음 → 명시 체크리스트로 *강제화*.)
