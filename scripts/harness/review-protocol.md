...
LLM이 LLM을 판단하는 self-critique는 약하다(연구: `docs/harness-engineering/RESEARCH_HARNESS_AND_MULTIAGENT.md` §2). 의견을 내기 전에 **결정론적 외부 신호**부터 확보하라:
```bash
...
- **인터페이스 우선 위반**: Service·Repository·Gateway를 인터페이스 없이 구현 클래스에 직접 의존
- **트랜잭션 위반**: `@Transactional` 메서드 안에서 외부 API 호출(보상 트랜잭션이어야 함) / Tier 경계 위반(Tier1 돈·유저 vs Tier2 온톨로지)
- **캡슐화 위반**: Entity setter 남발, Tell-Don't-Ask 위반, 원시 타입 노출(좌표를 `double` 2개로 — `Coordinate` VO 무시 등)
