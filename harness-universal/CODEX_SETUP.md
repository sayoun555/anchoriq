...
**하네스 원본(설치 소스)** — 이 머신 어딘가의 `harness-universal/` 폴더:
- 기본 경로: `/Users/sanghyunyoun/anchoriq/harness-universal/`
- 못 찾으면 이 명령으로 검색:
  ```bash
  find ~ /Users -maxdepth 6 -type f -name CODEX_SETUP.md -path '*harness-universal*' 2>/dev/null | head -1
  # → 나온 경로의 디렉토리가 HARNESS_SRC
  ```

**설치 후 — 이 프로젝트 안에 생기는 것**(전부 repo 루트 `git rev-parse --show-toplevel` 기준):
| 경로 | 무엇 | 너가 손대나 |
|------|------|------------|
...
| `AGENTS.md`(루트) | Codex가 자동으로 읽는 지침 | Step 4 |
...
## 1. 하네스 설치
`HARNESS_SRC` = 위 "파일 위치"의 원본 폴더 경로(기본 `/Users/sanghyunyoun/anchoriq/harness-universal`, 없으면 거기 `find` 명령으로 찾기).
```bash
HARNESS_SRC="/Users/sanghyunyoun/anchoriq/harness-universal"   # 위 find로 확인한 실제 경로
[ -f "$HARNESS_SRC/install.sh" ] || { echo "HARNESS_SRC 못 찾음 — '파일 위치'의 find 명령으로 경로부터 확정"; exit 1; }
